...
Create the deployment failure screen. Show FAILED state, exact failing step, actionable error detail, exit code when available, relevant log excerpt, timestamp, deployment ID and safe recovery actions such as Retry, Inspect Logs, Change Configuration and Rollback when applicable. Avoid generic failure banners.
