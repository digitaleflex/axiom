...
Create the application metrics screen. Show meaningful CPU, memory, runtime health, request/response or application-level telemetry only where supported, deployment correlation and time ranges. Avoid decorative charts; every visualization must communicate an operational decision or state.
