...
Create the live deployment screen during execution. Make the pipeline the dominant visual object. Show completed/current/pending steps, elapsed time, live event/log stream, current state, build image, runtime/container information when available, and an always-accessible Abort control. Current phase should be visually obvious.
